    ...
    try:
        ...
        try:
            if db.query(Guest).count() == 0:
                ...
                reservations = [
                    Reservation(guest_id=1, room_id=1, check_in_date=date(2024, 12, 1), 
                               check_out_date=date(2024, 12, 3), total_amount=100.00, status=ReservationStatus.CONFIRMED),
                    Reservation(guest_id=2, room_id=3, check_in_date=date(2024, 12, 5),
                               check_out_date=date(2024, 12, 7), total_amount=160.00, status=ReservationStatus.CONFIRMED),
                    Reservation(guest_id=3, room_id=5, check_in_date=date(2024, 12, 10),
                               check_out_date=date(2024, 12, 12), total_amount=300.00, status=ReservationStatus.CONFIRMED),
                    Reservation(guest_id=4, room_id=2, check_in_date=date(2024, 12, 15),
                               check_out_date=date(2024, 12, 17), total_amount=100.00, status=ReservationStatus.PENDING)
                ]
